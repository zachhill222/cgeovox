...
namespace gv::mesh {
	...
	class LogicalMesh : public BasicMesh<Node_t,Element_t,Face_t> {
		/////////////////////////////////////////////////
		...
		// friend std::ostream& operator<<(std::ostream& os, const LogicalMesh<U,Element_u,Face_u> &mesh);
	...
	public:
		LogicalMesh(const Mesh_t &mesh) : Mesh_t(),
			_parent(mesh),
		...
		// Count methods
	    size_t nNodes_Locked() const override {
		...
		ElementIterator_t begin() const override {
			return ElementIterator_t(const_cast<LogicalMesh<Node_t,Element_t,Face_t>*>(this),
	...
	static_assert(BasicMeshType< LogicalMesh<BasicNode<gv::util::Point<3,double>>, BasicElement, BasicElement >>,
		"LogicalMesh is not a BasicMeshType with default template parameters.");
}