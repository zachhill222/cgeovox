...
namespace gv::util{
	...
	private:
		...
		struct Node
		{
			...
			const Box<dim,double> bbox;
			Node(const Node* parent, const Point<dim,double> &v1, const Point<dim,double> &v2) : parent(parent), bbox(Box<dim,double>(v1,v2)) {data_idx = new size_t[n_data];}
			~Node()
			{
		...
		virtual bool is_data_valid(Box<dim,double> const &box, const data_t &data) const {return false;}
		Node* root = NULL;
	...
	public:
		...
		BasicOctree(const Box<dim,double> &bbox) {root = new Node(NULL, bbox.low(), bbox.high());}
		~BasicOctree() {delete root;}

		void insert(const data_t &val)
		{
			if (recursive_contains(root, val)) {return;}
			if (not is_data_valid(root->bbox, val)) {return;}
			_data.push_back(val);
	...
	class PointOctree : public BasicOctree<Point<dim,T>, dim, false, n_data>
	{
	public:
		PointOctree() : BasicOctree<Point<dim,T>, dim, false, n_data>() {}
		PointOctree(const Box<dim,double> &bbox) : BasicOctree<Point<dim,T>, dim, false, n_data>(bbox) {}
	private:
		virtual bool is_data_valid(Box<dim,double> const &box, Point<dim,T> const &data) const {return box.contains(data);} 
	};
