...
namespace gv::util
{
	...
	public:
		...
		PointOctree() : //if bounding box is unknown ahead of time
			gv::util::BasicOctree<Data_t, dim, n_data>(1024) {}

		PointOctree(const gv::util::Box<dim> &bbox, const size_t capacity=1024) :
			gv::util::BasicOctree<Data_t, dim, n_data>(bbox, capacity) {}
		...
		constexpr PointOctree(const PointOctree &other, const gv::util::Box<dim>& new_bbox) :
			gv::util::BasicOctree<gv::util::Point<dim,double>, dim, n_data>(new_bbox, other.capacity())
		{
