...
namespace gv::pde
{
	...
	public:
		...
		void solve(int verbose=0)
		{
			...
			//initialize matrices and vectors
			gv::charms::CharmsGalerkinMatrixConstructor matrix_constructor(mesh);
			matrix_constructor.epsilon = epsilon;
			...
			if (verbose>0) {std::cout << "making stiffness matrx " << std::flush; start = std::time(nullptr);}
			...
			//apply Dirichlet BC
			if (verbose>0) {std::cout << "applying strong Dirichlet BC " << std::flush; start = std::time(nullptr);}
			matrix_constructor.set_dirichlet_bc(stifMat, boundary);
			for (size_t i=0; i<boundary.size(); i++)
			{
				size_t b_idx = mesh.basis_active2all[boundary[i]];
