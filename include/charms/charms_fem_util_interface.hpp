...
namespace gv::charms
{
	...
	public:
		...
		double epsilon=-1; //convenient place to store this parameter for the smoothed out interface

		//evaluate smoothed Heaviside function
		static double heaviside(const double phi, const double eps)
		{
			assert(eps>0);
			...
			const double r = phi/eps;
			return 0.5*(1 + r + 0.159154943091895*std::sin(3.141592653589793*r));
		}

		//evalutate smoothed dirac delta
		static double dirac_delta(const double phi, const double eps)
		{
			assert(eps>0);
			...
			const double r = 1/eps;
			return 0.5*r*(1 + std::cos(3.141592653589793*phi*r));
		}
		...
		void integrate_fun(Eigen::Vector<double,n> &rhs, const ScalarFun_t &rhs_fun,
			...
			const std::vector<size_t>& elem_active2all,
			const std::vector<size_t>& elem_all2active,
			const ScalarFun_t& sgndist) const
		{
			const ScalarFun_t HEAVISIDE = [this, sgndist](Point_t point) -> double {return heaviside(sgndist(point), this->epsilon);};
		...
		{
			const ScalarFun_t DELTA = [this, sgndist](Point_t point) -> double {return dirac_delta(sgndist(point), this->epsilon);};
			...
			for (size_t e_idx=0; e_idx<elem_active2all.size(); e_idx++)
			{
				...
				for (int i=0; i<ELEM.cursor_basis_s; i++)
				{
					...
					//integrate FUN_i * FUN_i
					std::function<double(Point_t)> fun_i = [&FUN_i](Point_t point) -> double {return FUN_i.eval(point);};
					double integral = penalty * gauss_quad_interface(fun_i, fun_i, DELTA, ELEM.bbox());
					...
					for (int j=i+1; j<ELEM.cursor_basis_s; j++)
					{
						...
						//integrate FUN_i * FUN_j
						std::function<double(Point_t)> fun_j = [&FUN_j](Point_t point) -> double {return FUN_j.eval(point);};
						integral = penalty * gauss_quad_interface(fun_i, fun_j, DELTA, ELEM.bbox());
